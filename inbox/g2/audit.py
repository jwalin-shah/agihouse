"""Deterministic policy gate + append-only audit log for inbox/g2 actions.

Every time the agent considers acting it asks :func:`gate(action, **ctx)`. The
gate returns a :class:`Decision`, and the consideration is written to
``~/.inbox/audit.log`` as one JSONL row. The model never overrides the gate —
these are hard rules from ``inbox/g2/policy.yaml`` (hot-reloaded on mtime
change so the operator can edit policy live during a demo).

Usage::

    from inbox.g2 import audit

    d = audit.gate("send_imessage", handle="+15551112222", text="...")
    if d.allow:
        services.imsg_send(...)
        audit.mark_fired("send_imessage", handle="+15551112222")
    else:
        # gate has already logged the suppression
        pass
"""
...
_RECALL_ACTIONS = {"recall", "recall_person"}
...
_DEFAULT_POLICY: dict = {
    ...
    "denied_actions": ["send_email", "delete_anything", "share_externally"],
    "restraint": {
...
def reset_state_for_tests() -> None:
    """Clear in-memory restraint counters and force a policy reload.

    Tests that toggle ``policy.yaml`` should call this before each scenario.
    """
    global _policy_cache, _policy_mtime
    _policy_cache = None
    _policy_mtime = None
    _recall_last_fired.clear()
    _proposal_window.clear()


# ── In-memory restraint state ────────────────────────────────────────────

_recall_last_fired: dict[str, float] = {}
_proposal_window: deque[float] = deque(maxlen=100)
...
def _log_path() -> Path:
    raw = policy().get("log_path", "~/.inbox/audit.log")
...
def gate(action: str, **ctx: Any) -> Decision:
    """Return a :class:`Decision` and log the consideration. NEVER raises."""
    pol = policy()
    ...
    person = (ctx.get("person") or ctx.get("name") or "").strip()
    ...
    blob = " ".join(
        ...
        if k in {"transcript_chunk", "snippet", "summary", "text", "body"}
    ).lower()
    ...
    if action in _RECALL_ACTIONS and person:
...
def summary() -> dict:
    """Aggregate audit.log into counts so the demo panel can show 'considered N, fired M'."""
    path = _log_path()
    if not path.exists():
        return {"total": 0, "by_decision": {}, "by_action": {}, "top_suppression_reasons": []}
