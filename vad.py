"""On-device VAD gate (Silero) for the voice-trigger pipeline.

Audio path goal:

    raw mic frames  ->  SpeechGate  ->  speech-only segments  ->  whisper

Today voice_trigger.py runs whisper over *every* audio frame, then regex-
matches transcripts. That sends ambient room noise, background TV, and
other people's speech through ASR for no reason — wasted CPU and a wider
text surface than necessary in transcript history.

This gate scores each 32 ms frame with a small (~1.8 MB) Silero VAD net
and emits only contiguous speech segments. Drop those into whisper and
you get the same triggers with far less overheard text on the wire.
...
File usage (sanity-check on a wav clip):

    python vad.py clip.wav
        ...
        if not self._in_speech:
            if is_speech:
                ...
                if self._speech_run_ms >= self.min_speech_ms:
                    # Confirmed: backdate segment start to where the run began.
                ...
                # Speech run broken before confirmation — drop the pre-roll.
...
def _main() -> int:
    ...
    if len(sys.argv) != 2:
        print("usage: python vad.py <16kHz_mono.wav>", file=sys.stderr)
        return 2
    gate = SpeechGate(keep_audio=False)
    n = 0
    for frame in iter_wav_frames(sys.argv[1]):
        for seg in gate.feed(frame):
            print(f"[vad] speech {seg.start_ms}-{seg.end_ms} ms ({seg.duration_ms} ms)")
            n += 1
    for seg in gate.flush():
        print(f"[vad] speech {seg.start_ms}-{seg.end_ms} ms ({seg.duration_ms} ms) (flush)")
        n += 1
    print(f"[vad] {n} segment(s)")
    return 0
