...
echo ""
echo "Step 2: Optional, start ngrok in a separate terminal:"
...
cd "$ROOT/inbox"
INBOX_SERVER_HOST=0.0.0.0 uv run python inbox_server.py &
INBOX_PID=$!
sleep 3
...
echo "  Inbox LAN:      http://$LAN_IP:9849"
echo "  G2 app (Vite):  http://localhost:5173"
echo "  G2 app LAN:     http://$LAN_IP:5173"
echo "  Demo panel:     demo/index.html (already opened)"
echo ""
echo "  G2 Simulator:   cd g2-app && npm run simulate"
echo "  QR for glasses: cd g2-app && npx evenhub qr --url http://$LAN_IP:5173"
echo ""
